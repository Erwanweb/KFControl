...
<plugin key="Keyfob" name="AC Keyfob control" author="Erwanweb" version="0.0.2" externallink="https://github.com/Erwanweb/KFControl.git">
    ...
    <params>
        ...
        <param field="Mode1" label="Slave device for Carre (csv list of idx)" width="200px" required="false" default=""/>
        <param field="Mode2" label="Slave device for Rond (csv list of idx)" width="200px" required="false" default=""/>
        <param field="Mode3" label="Slave device for croix (csv list of idx)" width="200px" required="false" default=""/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        # build lists of slaves devices
        self.Carre = parseCSV(Parameters["Mode1"])
        Domoticz.Debug("Slaves devices for Carre = {}".format(self.Carre))
        self.Rond = parseCSV(Parameters["Mode2"])
        Domoticz.Debug("Slaves devices for Rond = {}".format(self.Rond))
        self.Croix = parseCSV(Parameters["Mode3"])
        Domoticz.Debug("Slaves devices for Croix = {}".format(self.Croix))


        # splits additional parameters for alarm control
        params = parseCSV(Parameters["Mode4"])
        if len(params) == 2:
            self.Alarmcontrol = CheckParam("Alarm Log IDX is ",params[0],0)
            Domoticz.Debug("Alarm Control device = {}".format(self.Alarmcontrol))
            self.Alarmlog = CheckParam("Alarm Log IDX is ",params[1],0)
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if (Unit == 5):
            ...
            if Devices[4].nValue == 1:
                DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Commande depuis keyfob {} - Alarme Desarmee".format(
                            self.Alarmlog,Parameters["Mode5"]))
                Domoticz.Debug("updating alarm log - Alarm turned Off by Keyfob")
                DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set Level&level=10".format(self.Alarmcontrol))
        ...
        if (Unit == 6):
            ...
            if Devices[4].nValue == 1:
                DomoticzAPI(
                    "type=command&param=udevice&idx={}&nvalue=0&svalue=Commande depuis keyfob {} - Activation Protection Totale".format(
                        self.Alarmlog,Parameters["Mode5"]))
                Domoticz.Debug("updating alarm log - Alarm turned Off by Keyfob")
