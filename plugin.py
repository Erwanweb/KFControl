...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 6 not in Devices:
            Domoticz.Device(Name="Plus", Unit=6, TypeName="Switch", Image=13, Used=1).Create()
        ...
        self.KeyfobName = parseCSV(Parameters["Mode 5"])
        Domoticz.Debug("Keyfob Name = {}".format(self.KeyfobName))
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if (Unit == 4):
            ...
            if (Devices[4].sValue == "on"):
                Domoticz.Debug("Triangle push on")
                self.LastOrder = datetime.now()
            ...
            if (Devices[5].sValue == "on"):
                ...
                if Devices[4].nValue == 1:
                    for idx in self.Alarmlog:
                        DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Commande alarme depuis keyfob () - Alarme Desarmee".format(idx,self.KeyfobName))
        ...
        if (Unit == 6):
            Devices[61].Update(nValue = 0,sValue = str(Level))
            if (Devices[6].sValue == "on"):
                ...
                if Devices[4].nValue == 1:
                    for idx in self.Alarmlog:
                        DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Commande alarme depuis keyfob () - Protection totale".format(idx,self.KeyfobName))
    ...
    def onHeartbeat(self):
        ...
        if self.LastOrder + timedelta(seconds = 30) <= now:
            if Devices[1].nValue == 1:
                Devices[1].Update(nValue = 0,sValue = "off")
            if Devices[2].nValue == 1:
                Devices[2].Update(nValue = 0,sValue = "off")
